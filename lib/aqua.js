/*! Aqua v20.0.0 by @ignatiusmb - https://mauss.dev
...
(global = global || self, global.Aqua = factory());
}(this, (function () { 'use strict';

var create = {
  icon: (name, tooltip) => {
    let cbName = 'Aqua.code.callbacks';
    if (name === 'clipboard') cbName += '.copy';else if (name === 'list') cbName += '.toggle';
...
const codeCallback = {
  copy: () => {
    ...
    try {
      if (document.execCommand('copy')) textField.textContent = 'Copied to clipboard!';else textField.textContent = 'Copy failed';
    } catch (err) {
...
function highlight(source, langInput) {
  ...
  return lang ? prism.highlight(source, prism.languages[lang], langInput) : source.replace(/[&<>]/g, c => ({
    '&': '&amp;',
    '<': '&lt;',
    '>': '&gt;'
  })[c]);
